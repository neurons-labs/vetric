...
/// Buckets configuration for a [`Histogram`](crate::Histogram) or a [`Family`](crate::Family) of
/// histograms.
#[derive(Debug, Clone, Copy)]
...
impl Buckets {
    ...
    /// consist of `range.start`, `range.start + step`, ..., until `range.end` (potentially
    /// inclusive).
    ///
    /// # Panics
    ///
    ...
    /// will consist of `range.start`, `range.start * factor`, ... until `range.end` (potentially
    /// inclusive).
    ///
    /// # Panics
    ///
...
const fn compare_f64(lhs: f64, rhs: f64) -> Option<cmp::Ordering> {
    // Since the endianness is the same for `f64` and `u64`, we can use fixed masks regardless of
    // it.
