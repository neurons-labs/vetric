...
//!   are values keyed by a set of labels. See [`EncodeLabelValue`] and [`EncodeLabelSet`] derive
//!   macros for more info on labels.
//! - To define metrics, a group of logically related metrics is grouped into a struct and the
//!   [`Metrics`](trait@Metrics) trait is [derived](macro@Metrics) for it. This resolves full metric
//!   names and records additional metadata, such as help (from doc comments), unit of measurement
//!   and [`Buckets`] for histograms.
//! - Metric groups are registered in a [`Registry`], which then allows to
//!   [encode](Registry::encode()) metric data in the OpenMetrics text format. Registration can be
//!   automated using the [`register`] attribute, but it can be manual as well.
...
//!
//! ```
//! # use vetric::{Gauge, Global, Metrics, MetricsCollection, Registry};
//! #[derive(Debug, Clone, Metrics)]
//! pub(crate) struct MyMetrics {
//! #   pub gauge: Gauge<u64>,
...
//!
//! ```
//! # use vetric::{Gauge, Metrics, Registry};
//! #[derive(Debug, Clone, Metrics)]
//! pub(crate) struct MyMetrics {
//! #   pub gauge: Gauge<u64>,
//!     // defined in a previous code sample
...
///
/// ```
/// use vetric::{Gauge, Global, Metrics};
///
...
///
/// ```
/// use vetric::{Collector, Gauge, Global, Metrics};
///
/// #[derive(Debug, Metrics)]
...
/// ```
pub use vetric_macros::register;
/// Derives the [`EncodeLabelSet`] trait for a type, which encodes a set of metric labels.
///
/// The type for which the trait is derived must not have generics (lifetimes, type params or
/// const params). The macro can be configured using `#[metrics()]` attributes.
///
...
///
/// This attribute works similarly to `skip_serializing_if` in `serde` – if the function it
/// points to returns `true` for the field value, the field will not be encoded as a label.
///
/// `Option` fields are skipped by default if they are `None` (i.e., they use `skip =
/// Option::is_none`).
///
...
/// use vetric::{EncodeLabelSet, EncodeLabelValue};
///
...
/// }
/// ```
pub use vetric_macros::EncodeLabelSet;
...
/// ## `rename_all`
///
/// **Type:** string; one of `lowercase`, `UPPERCASE`, `camelCase`, `snake_case`,
...
/// Caveats:
///
/// - `rename_all` assumes that original variant names are in `PascalCase` (i.e., follow Rust
///   naming conventions).
/// - `rename_all` requires original variant names to consist of ASCII chars.
/// - Each letter of capitalized acronyms (e.g., "HTTP" in `HTTPServer`) is treated as a
///   separate word. E.g., `rename_all = "snake_case"` will rename `HTTPServer` to
///   `h_t_t_p_server`. Note that [it is recommended][clippy-acronyms] to not capitalize
///   acronyms (i.e., use `HttpServer`).
/// - No spacing is inserted before numbers or other non-letter chars. E.g., `rename_all =
///   "snake_case"` will rename `Status500` to `status500`, not to `status_500`.
///
/// # Variant attributes
///
/// ## `name`
...
/// **Type:** string
///
/// Specifies the name override for a particular enum variant when used with the `rename_all`
...
/// }
/// ```
pub use vetric_macros::EncodeLabelValue;
/// Derives the [`Metrics`](trait@Metrics) trait for a type.
///
/// This macro must be placed on a struct with named fields and with no generics (lifetimes,
/// type params or const params). Each field will be registered as metric or a family of
/// metrics. The macro can be configured using `#[metrics()]` attributes.
///
/// # Container attributes
///
/// ## `prefix`
...
/// **Type:** string
///
/// Specifies a common prefix for all metrics defined in the type. If specified, the prefix
/// will be prepended together with a `_` separator to a field name to get the metric name.
/// Note that the metric name may be additionally transformed depending on the unit and metric
/// type.
///
/// # Field attributes
///
...
///
/// Specifies buckets for a [`Histogram`] or a [`Family`] of `Histogram`s. This attribute is
/// mandatory for these metric types and will result in a compile-time error if used on
/// counters / gauges.
///
/// ## `unit`
///
...
///
/// Specifies unit of measurement for a metric. Note that specifying a unit influences the
/// metric naming.
///
/// ## `labels`
///
...
///
/// Specifies label names for a [`LabeledFamily`]. This attribute is mandatory for
/// [`LabeledFamily`] and will result in a compile-time error if used with other metric types.
/// The number of label names must match the number of label values in the `LabeledFamily`
/// (i.e., its type).
///
/// # Examples
///
/// See crate-level docs and other crate docs for the examples of usage.
pub use vetric_macros::Metrics;
