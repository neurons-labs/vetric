...
#[non_exhaustive]
pub enum Format {
    /// [OpenMetrics text format][om]. This is the original format produced by
    /// [`prometheus-client`], which this library is based upon.
    ///
    ...
    OpenMetrics,
    /// [Prometheus text format][prom]. Since it's quite similar to the OpenMetrics format, it's
    /// obtained by a streaming transform of OpenMetrics-encoded metrics that removes `_total`
    /// suffixes from reported counter values and removes the `# EOF` terminator.
    ///
    /// [prom]: https://prometheus.io/docs/instrumenting/exposition_formats/
    Prometheus,
    /// OpenMetrics text format as understood by Prometheus.
    ///
    /// Prometheus *mostly* understands the OpenMetrics format (e.g., enforcing no empty lines for
    /// it). The notable exception is counter definitions; OpenMetrics requires
