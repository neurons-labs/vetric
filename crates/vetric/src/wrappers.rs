...
/// Gauge metric.
///
/// Gauges are integer or floating-point values that can go up or down. Logically, a reported gauge
/// value can be treated as valid until the next value is reported.
///
...
/// Histogram metric.
///
/// Histograms are floating-point values counted in configurable buckets. Logically, a histogram
/// observes a certain probability distribution, and observations are transient (unlike gauge
/// values).
///
...
/// - The number of labels must match the number of label names and the constant param of
///   `LabeledFamily` (which is set to 1 by default). E.g., for two labels you should use
///   `LabeledFamily<_, _, 2>`.
///
/// # Examples
///
...
///
/// ```
/// use vetric::{Counter, LabeledFamily, Metrics};
/// # use vetric::{Format, Registry};
///
...
///
/// ```
/// # use vetric::{Buckets, Format, Histogram, LabeledFamily, Metrics, Registry};
    ...
    M: BuildMetric,
{
    ...
    /// Checks whether this family contains a metric with the specified labels. This is mostly
    /// useful for testing.
...
mod tests {
    ...
    fn standard_family_is_easy_to_deadlock() {
        ...
        thread::spawn(move || {
            ...
            // `get_or_create()` provides a read guard for the internal map, and creating a new
            // metric requires a write lock on the same map.
    ...
    fn family_accesses_are_not_deadlocked() {
        ...
        // We circumvent deadlocking problems by using a *frozen map* (one that can be updated via a
        // shared ref). See its docs for more details. As an added bonus, we can use
        // indexing notation instead of clunky methods!
    }
}
