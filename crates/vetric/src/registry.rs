...
use crate::{
    ...
    descriptors::{FullMetricDescriptor, MetricDescriptor, MetricGroupDescriptor},
...
impl MetricsCollection {
    ...
    ///
    /// By default, [`Global`] metrics are eagerly collected into a [`Registry`]; i.e., metrics will
    /// get exported even if they were never modified by the app / library logic. This is
    /// *usually* fine (e.g., this allows getting all metrics metadata on the first scrape), but
    /// sometimes you may want to export only metrics touched by the app / library logic. E.g.,
    /// you have a single app binary that exposes different sets of metrics depending on
    /// configuration, and exporting all metrics is confusing and/or unacceptably bloats
    /// exported data size.
    ///
    /// `lazy()` solves this issue. It will configure the created `Registry` so that `Global`
    /// metrics are only exported after they are touched by the app / library logic. Beware that
    /// this includes being touched by an eager `MetricsCollection` (only metrics actually
    /// included into the collection are touched).
    pub fn lazy() -> Self {
        Self {
            is_lazy: true,
            ..Self::default()
...
/// Metrics registry.
///
/// A registry collects [`Metrics`] and [`Collector`]s defined in an app and libs the app depends
/// on. Then, these metrics can be scraped by calling [`Self::encode()`].
///
/// # Collecting metrics
///
/// You can include [`Metrics`] and [`Collector`]s to a registry manually using
/// [`Self::register_metrics()`] and [`Self::register_collector()`]. However, this can become
/// untenable for large apps with a complex dependency graph. As an alternative, you may use
/// [`register`](crate::register) attributes to mark [`Metrics`] and [`Collector`]s that should be
/// present in the registry, and then initialize the registry with [`MetricsCollection`] methods.
///
/// ```
/// use vetric::{Buckets, Global, Histogram, Metrics, MetricsCollection, Registry, Unit};
...
/// ```
///
/// Registered metrics can be filtered. This is useful if you want to avoid exporting certain
/// metrics under certain conditions.
///
/// ```
/// # use vetric::{MetricsCollection, Registry};
...
///
/// ```should_panic
/// # use vetric::{Collector, Global, Gauge, Metrics, MetricsCollection, Unit};
/// #[derive(Debug, Metrics)]
/// pub(crate) struct AppMetrics {
...
// TODO(VIVI): remove usage of the MetricVistor.
#[derive(Debug)]
enum MetricsVisitorInner<'a> {
    Registry(&'a mut Registry),
    Collector(&'a mut Vec<(MetricDescriptor, Box<dyn Metric>)>),
}
...
impl<'a> MetricsVisitor<'a> {
    pub(crate) fn for_collector(metrics: &'a mut Vec<(MetricDescriptor, Box<dyn Metric>)>) -> Self {
