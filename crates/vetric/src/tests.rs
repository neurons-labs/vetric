...
fn lazy_metrics_registration() {
    // Metric names clash with `TestMetrics` above, but because we use group filtering in both
    // cases, this is fine.
    #[derive(Debug, Metrics)]
    #[metrics(crate = crate, prefix = "test")]
...
fn labeled_family_with_multiple_labels() {
    ...
    #[metrics(crate = crate, prefix = "test")]
    struct MetricsWithLabels {
        /// Counters labeled by a tuple with the corresponding 2 label names specified via an
        /// attribute.
