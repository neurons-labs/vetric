...
/// Maps a set of labels from the storage format (i.e., how labels are stored in a
/// [`Family`](crate::Family)) to the encoding format, which is used when [exporting
/// metrics](crate::Registry::encode()).
...
// We cannot use a reference directly because there are no blanket implementations for
// `EncodeLabelSet` and `EncodeLabelValue`.
#[derive(Debug)]
